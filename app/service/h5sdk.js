...
const aiaiusdk = (function() {
  ...
  aiaiusdk.pay = function(_paydata, lisenter) {
    ...
    const payStr = JSON.stringify(_paydata);
    if (aiaiusdk.paying) {
      return;
    }
    setTimeout(function() {
    ...
    if (aiaiusdk.join == 2) {
      ...
      aiaiusdk.jsonp(psyStyleUrl + '?open_id=' + params.open_id, function(ret) {
        if (ret.code == 200 && ret.data.join == 2) {
          ...
          const msg = { action: event, source, data };
  ...
  aiaiusdk.jsonp = function(url, callback) {
    ...
    const cbname = 'aiaiusdk.jsonp.' + name; // 作为jsonp函数的属性
    if (url.indexOf('?') === -1) {
      url += '?callback=' + cbname;
    } else {
      url += '&callback=' + cbname;
